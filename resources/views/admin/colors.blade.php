...
@section('content')
        ...
        <div class="card-body">
          ...
          <form action="{{ route('admin.color.store') }}" method="POST">
            ...
            <div class="mb-3">
              ...
              <input type="color" name="color_code" class="form-control border-2 form-control-lg" />
            </div>
            <div class="mb-3">
            ...
            <tbody>
              @forelse ($colors as $color)
                <tr id="tr{{ $color->id }}">
                  ...
                  <td>
                    <button class="btn btn-xs btn-success" data-bs-target="#editModal" data-bs-toggle="modal" data-color="{{ $color }}">
                      <ion-icon name="create-outline"></ion-icon>
                    </button>
                    <button class="btn btn-xs btn-danger" data-bs-target="#deleteModal" data-bs-toggle="modal" onclick="$('#id-for-destroy').val('{{ $color->id }}');">
      ...
      <div class="modal-content rounded-0">
        ...
        <form action="{{ route('admin.color.update') }}" method="POST">
          ...
          <div class="modal-body">
            ...
            <div class="mb-3">
              <label class="form-label">اللون: </label>
              <input type="color" name="color_code" class="form-control border-2 form-control-lg" id="e-color_code" />
            </div>
          </div>
  ...
  <div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content rounded-0">
        <div class="modal-header">
          <h5 class="modal-title">هل حقا تريد حذف اللون؟</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-footer">
          <input type="hidden" name="id" value="" id="id-for-destroy" />
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">غلق</button>
          <button type="button" class="btn btn-danger" data-bs-dismiss="modal" onclick="destroyColor();">حذف</button>
        </div>
      </div>
    </div>
  </div>
...
@section('scripts')
  <script type="text/javascript">
    function destroyColor() {
      ...
      $.post('{{ route('admin.color.destroy') }}', {id:id, _token:'{{ csrf_token() }}'}, function(res) {
        $('#tr'+id).remove();
      }, 'json');
    }
    ...
    $('#editModal').on('show.bs.modal', function (event) {
      ...
      var modal = $(this);
      modal.find('#e-name_ar').val(color.name_ar);
      modal.find('#e-name_en').val(color.name_en);
      modal.find('#e-name_fr').val(color.name_fr);
      modal.find('#e-color_code').val(color.color_code);
      modal.find('#e-id').val(color.id);
    });
  </script>
@endsection